    ...
    onEvent : (CalculatorViewModel.UIEvent) -> Unit)
{
    Column(
        ...
        MeasurementsSliderView(min = 120f, max = 210f, steps = 90, sliderPosition = heightCm){ height ->
            onEvent(CalculatorViewModel.UIEvent.ChangeHeight(heightCm = height))
        }
        ...
        MeasurementsSliderView(min = 30f, max = 150f, steps = 240, sliderPosition = weightKg){ weight ->
            onEvent(CalculatorViewModel.UIEvent.ChangeWeight(weightKg = weight))
        }
...
@Preview
@Composable
private fun BodyMeasurementsPreview() {
